...
class EmbeddingRabbitPublisherAdapterTest {

    @Mock
    private RabbitTemplate rabbitTemplate;

    @InjectMocks
    private EmbeddingRabbitPublisherAdapter adapter;
    ...
    void publish_sendsMessageWithDocumentIdHeader() {
        ...
        EmbedTask task = new EmbedTask(docId);

        adapter.publish(task);

        verify(rabbitTemplate).convertAndSend(
                eq(QueueConfig.EXCHANGE),
                eq(QueueConfig.EMBED_ROUTING_KEY),
                eq(task),
                mppCaptor.capture()
        );
